...
class OperacionEntrada(BaseModel):
    centro: str
    minutos_preparacion: int = Field(ge=0, default=0)
    minutos_unitario: int = Field(ge=0, default=0)
...
class PeticionCalculo(BaseModel):
    ...
    merma_pct: Decimal = Decimal("10")
    indirectos_pct: Decimal = Decimal("10")
    margen_pct: Decimal = Decimal("35")
